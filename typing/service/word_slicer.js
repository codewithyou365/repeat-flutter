...
const Game = {
    ...
    selectRole: async function (args) {
        try {
    ...
    clear: async function () {
        ...
        if (hcp > 0 && hcp <= 1 && this.originContent.length > 0) {
            let chars = this.originContent.split('');
            const hideCount = Math.round(chars.length * hcp);
            let indexes = Array.from({length: chars.length}, (_, i) => i);
    ...
    submit: async function (args) {
        const userId = args.userId;
        const selectedIndexed = args.data || [];
        ...
        if (userColorIndex !== -1) {
            ...
            selectedIndexed.forEach(index => currentSelections.add(index));
            this.colorIndexToSelectedContentIndex[userColorIndex] = Array.from(currentSelections).sort((a, b) => a - b);
        }
    ...
    checkFinishCondition: async function () {
    ...
    _getColorIndexToWords: function () {
        const result = [];
        for (let i = 0; i < 3; i++) {
...
// 内部工具：提取连续索引块 (由原 Dart 代码转化)
    _extractConsecutiveWord: function (indexes, colorIndex) {
        ...
        for (let i = 1; i < sorted.length; i++) {
            if (sorted[i] === end + 1) {
                ...
                ranges.push({start: start, end: end, colorIndex: colorIndex});
                start = end = sorted[i];
            }
        }
    ...
    setResult: async function () {
        ...
        selectedWords.forEach((selected, idx) => {
        ...
        this.userIds.forEach(userId => {
            ...
            if (userColorIdx !== -1) {
                ...
                if (finalScore !== 0) {
                    Util.gameScoreInc(userId, finalScore, finalScore > 0 ? 'i:obtainedInTheGame' : 'i:deductedInTheGame');
                }
            }
        });
    ...
    resetWordSlicerText: async function (args) {
        try {
            const userId = args.userId;
            const adminId = Util.adminId();
            const adminEnable = Util.adminEnable();
    ...
    setWordSlicerText: async function (args) {
        try {
            const userId = args.userId;
            const adminId = Util.adminId();
            const adminEnable = Util.adminEnable();
    ...
    getWordSlicerText: async function (args) {
        try {
            const verse = await Util.getVerse();
