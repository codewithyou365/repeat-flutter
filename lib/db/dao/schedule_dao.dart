...
class LearnConfig {
  ...
  LearnConfig({
    ...
    required this.learnCountPerGroup,
  });
  ...
  factory LearnConfig.fromJson(Map<String, dynamic> json) {
    return LearnConfig(
      ...
      learnCountPerGroup: json['learnCountPerGroup'],
    );
  }
...
// review learning config
class ReviewLearnConfig {
  String title;
  int level;
  int before;
  Date from;
  ...
  factory ReviewLearnConfig.fromJson(Map<String, dynamic> json) {
    return ReviewLearnConfig(
      title: json['title'],
      level: json['level'],
      before: json['before'],
      from: Date(json['from']),
...
class ScheduleConfig {
  ...
  factory ScheduleConfig.fromJson(Map<String, dynamic> json) {
    var elConfigsList = json['learnConfigs'] as List;
    var relConfigsList = json['reviewLearnConfigs'] as List;
...
abstract class ScheduleDao {
  ...
  @Query(
    "SELECT IFNULL(MIN(VerseReview.createDate),-1) FROM VerseReview"
  ...
  @Query(
    'SELECT ifnull(max(Verse.chapterIndex),0) FROM Verse'
    ' WHERE Verse.bookId=:bookId',
  )
  Future<int?> getMaxChapterIndex(int bookId);

  @Query(
    'SELECT ifnull(max(Verse.verseIndex),0) FROM Verse'
    ' WHERE Verse.bookId=:bookId'
    ' AND Verse.chapterIndex=:chapterIndex',
  )
  ...
  Future<List<VerseTodayPrg>> initToday() async {
    ...
    if (needToInsert) {
      ...
      var reviewLearnConfigs = scheduleConfig.reviewLearnConfigs;
      await initTodayRel(now, reviewLearnConfigs, todayPrg);
  ...
  List<VerseTodayPrg> refineEl(List<VerseTodayPrg> all, int index, LearnConfig config) {
  ...
  static Date getNextByProgress(DateTime now, int nextProgress) {
    ...
    if (nextProgress >= learnIntervalDays.length - 1) {
      ...
      return getNext(now, learnIntervalDays[nextProgress]);
    }
  }
