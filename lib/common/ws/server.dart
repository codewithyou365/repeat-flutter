...
class Server<User extends UserId> {
  ...
  Future<void> startLocal(int port, Future<User?> Function(HttpRequest request) auth, Future<void> Function(HttpRequest request) handleHttpRequest) async {
    final httpServer = await HttpServer.bind(InternetAddress.loopbackIPv4, port);
    await _init(httpServer, auth, handleHttpRequest);
  }
  ...
  Future<void> start(int port, Future<User?> Function(HttpRequest request) auth, Future<void> Function(HttpRequest request) handleHttpRequest) async {
    ...
    final httpServer = await HttpServer.bindSecure(InternetAddress.anyIPv4, port, context);
    await _init(httpServer, auth, handleHttpRequest);
  }
